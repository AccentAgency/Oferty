...
class CrearCuponDestacado extends React.Component{
        ...
        if(!this.state.titulo || !this.state.detalles || !this.state.descripcion || !this.state.ventaMin || !this.state.ventaEstd || !this.state.stock || !this.state.fechaVen ||
            ...
            if(validateForm(this.state.errors)) {
                if((this.state.value).length > 0){
                    ...
                    axiosInstance.post('/addCuponesDestacado',{
    ...
    handleChangeImagen3(event){
        ...
        axiosInstance.post('/uploadImage', formData,{
            ...
            this.setState({displayImage:'none'});
            this.setState({imagen3: res.data});
        })
        .catch((error) => {
            this.setState({displayImage:'none'});
            swal({title:"Ha ocurrido un error en subir la imagen",text:"Favor verifique su conexión a internet e intente de nuevo.",icon:"error",confirmButtonText: "Aceptar"});
...
export default CrearCuponDestacado;